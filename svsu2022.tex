...
\begin{questions}
	...
	\begin{solution}\\
		\begin{align*}
			(x+1)(x-1) - x^2 + 1 & = x^2 - 1 - x^2 + 1 = 0       \\
			x^0 - 1              & = 1 - 1       = 0             \\
			\sqrt{x^2} - x       & = \pm x - x = 0 \text{或} -2x
		\end{align*}
	\end{solution}
	...
	\begin{oneparchoices}
		\choice 10 \choice 11 \choice 16 \CorrectChoice 21 \choice 33
	\end{oneparchoices}
	...
	\begin{solution}\\
		\begin{minipage}{0.49\textwidth}
			\begin{tikzpicture}[scale=2]
				...
				\node[right] at (E) {E};
				\node[below] at (F) {F};
				\pic[draw, blue]{right angle = B--D--O};
				\pic[draw, blue]{right angle = A--E--O};
				\pic[draw, blue]{right angle = C--F--O};
				\draw[dashed] (O) -- ($(B)!(O)!(C)$) (O) -- ($(A)!(O)!(B)$) (O) -- ($(A)!(O)!(C)$);
				\draw[red, dashed] (A) -- (O) (B) -- (O) (C) -- (O);
			\end{tikzpicture}
		\end{minipage}
