\section[1977年高考数学试卷及答案(上海卷)理科]{1977普通高等学校招生考试（上海卷）\\\Huge{理科数学}}
...
\begin{questions}
	...
	\begin{parts}
		...
		\part 计算:
			...
			\begin{solution}
				...
				\end{align*}
			\end{solution}
			\part $\sqrt{-1} = \mathrm{i}$,验算$\mathrm{i}$是否方程$2x^4 + 3x^3 - 3x^2 + 3x - 5 = 0$的解.
			\begin{solution}
				将$i$代入方程得:
				\begin{align*}
					2\mathrm{i}^4 + 3\mathrm{i}^3 - 3x^2 + 3x - 5 & = 2 - 3\mathrm{i} + 3  + 3\mathrm{i} - 5 \\
					                            & = 0
				\end{align*}
				所以$\mathrm{i}$是原方程的解.
			\end{solution}
		\part 求证:$\displaystyle
			...
			\end{solution}
	\end{parts}
	\question[10] 在$\triangle{ABC}$中,$\angle{C}$的平分线交$AB$于$D$,过$D$作$BC$的平行线交$AC$于$E$,已知$BC=a, AC=b$,求$DE$的长.
	...
	\question[10] 正六棱锥$V-ABCDEF$的高为$2$cm,底面边长为$2$cm.
	...
	\begin{solution}
		...
		\begin{minipage}[b]{0.4\textwidth}
			...
			\begin{tikzpicture}[tdplot_main_coords]
				...
				\draw[very thin] (A) -- (B) -- (C) -- (D) -- (E) -- (F) -- cycle;
		...
		\begin{minipage}[b]{0.4\textwidth}
			...
			\begin{tikzpicture}[tdplot_main_coords]
				...
				\draw[very thin] (A) -- (B) -- (C) -- (D) -- (E) -- (F) -- cycle;
				\tkzDrawSegments(V,C  V,F)
			\end{tikzpicture}
			\captionof*{figure}{侧视图}
		\end{minipage}
		...
		\begin{center}
			...
			\begin{tikzpicture}[tdplot_main_coords, scale=2]
				...
				\draw[very thin] (F) -- (A) -- (B) -- (C);
				\draw[dashed, very thin] (C)-- (D) -- (E) -- (F);
			...
			\item 绘制数轴
			      ...
			      \begin{center}
				      \begin{tikzpicture}
					      ...
					      \draw[rounded corners=2pt](-4,0) |- (4,.5) -- (4,0);
					      \draw[rounded corners=2pt](3,0) |- (4.5,.8);
					      \draw[rounded corners=2pt](-2,0) |- (-4.5,.8);
					      \draw[pattern=north east lines, rounded corners=2pt](-4,0) rectangle (-2,.5);
					      \draw[pattern=north east lines, rounded corners=2pt](3,0) rectangle (4,.5);
					      \filldraw(-4,0) circle(1pt);
					      \filldraw(4,0) circle(1pt);
					      \filldraw[fill=white](-2,0) circle(1pt);
					      \filldraw[fill=white](3,0) circle(1pt);
	...
	\question[10]
	已知两定点$A(-4,0)$、$B(4,0)$,一动点$P(x,y)$与两定点$A$、$B$的连线$PA$、$PB$的斜率的乘积为$-\dfrac14$.求点$P$的轨迹方程,并把它化为标准方程,指出是什么曲线.
	\begin{solution}
		...
		\begin{center}
			\begin{tikzpicture}
				\begin{axis}[
					...
					\addplot[domain=-2:4, name path=A]{x+3};
					\addplot[domain=-2:4, name path=B]{x^2-2*x + 3};
					\addplot[gray] fill between [of=A and B,
							split,
							every segment no 0/.style={pattern=none},
							every segment no 1/.style={pattern=north east lines},
							every segment no 2/.style={pattern=none},
						];
