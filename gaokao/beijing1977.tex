...
\begin{questions}
	...
	\begin{parts}
		...
		\part 求椭圆 \( \frac{x^2}{a^2} + \frac{y^2}{b^2} = 1 \)绕 $x$轴旋转而成的旋转体体积.
			\begin{solution}[5cm]
	...
	\question[10]
	\begin{parts}
		\part 试用 \( \epsilon - \delta \)语言叙述\enquote{函数 \( f(x) \)在点 \( x = x_0 \)处连续}的定义.
			\begin{solution}[3cm]
				若对于任给的正数,总存在某一正数$\epsilon$,使得当$|x-x_0|<\delta$时, 总有$|f(x)-f(x_0)| <
					\epsilon$，则称函数$f(x)$在点$x_0$处连续.
			\end{solution}
		\part 试证明: 若 $f(x)$ 在点 $x = x_0$ 处连续, 且 $f(x_0) > 0$, 则存在一个 $x_0$ 的 $(x_0 − \delta, x_0 + \delta)$, 在这个邻域内, 处处有 $f(x) > 0$.
			\begin{proofsolution}[5cm]
