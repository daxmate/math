...
\begin{titlepage}
	...
	\begin{tikzpicture}[remember picture, overlay]
		...
		% 圆形装饰
		\draw[fill=cyan!30, draw=none] ($(current page.center)+(5,-5)$) circle (3cm);
		\draw[fill=magenta!40, draw=none, rounded corners=5pt] ($(current page.center)+(-5,-8)$) rectangle +(5cm, 3cm);
		...
		\node[anchor=north east, text=white, font=\large, rotate=90] at ($(current page.center)+(7,-3)$) {
			$e^{i\pi}+1=0$
		};
